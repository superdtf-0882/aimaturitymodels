...
// ---------------------------------------------------------------------
// CURRENCY BASIS (added 2026-08-26 on David's instruction).
//
// WHY THIS EXISTS. Two renderings of one model can each be perfectly
// current against their own source and still disagree, because their
// disagreement is temporal skew between two correct answers. RULE-6
// gives a rendering a currency obligation against ITS OWN source and
// that check works -- it is exactly what caught nothing here, because
// nothing was stale. DT2's ruling (briefs/2026-08-26-external-surface-
// currency/03-DT2-four-answers.md §2): declare the basis, and compare
// BASES rather than contents. Same basis, agreement is structural.
// Different bases, the possibility of disagreement is declared where a
// reader meets it.
//
// DERIVED, NEVER HAND-WRITTEN. Every value below is computed from the
// same constants the fetchers use. A hand-maintained basis line is the
// next stale claim -- this arc has already logged short_form.yml's
// citation drifting for a month behind the pin it described, and
// models.js's own comment going stale the moment that citation moved.
// If a pin changes, these strings change with it or they do not change
// at all, because there is no second copy.
//
// THE SDLC WHOLE-MODEL VIEW IS INTERNALLY SPLIT, and that is the case
// this field exists to make visible: getSdlcShortForm() fetches from
// main while getSdlcFullModel() fetches from the pinned commit, so one
// page renders two bases at once. PDLC and Prioritization are wholly
// pinned. Stated rather than left for a reader to discover.
const shortSha = (sha) => String(sha).slice(0, 7);

export const MODEL_CURRENCY = {
  sdlc: {

...
// The currency sentence below is COMPUTED, not written. It used to be
// literal prose here and went live on 2026-08-26 asserting "The site's
// Whole-Model Views and assessment pages read PINNED commits" -- false
// for the SDLC Whole-Model View, whose short-form cells come from main.
// That is the same one-basis-claim-about-a-two-basis-page defect the
// currency field had corrected on the page, restated here in the same
// commit. See lib/pins.js for why a shared CommonJS module was the fix
// rather than a reworded sentence.
const { digestCurrencyLine } = require("./pins");
...
This is a machine-readable digest of the AI-Native Maturity Model family, the Strata governance-layer model, the Enterprise Architecture OKF schema they're built on, and the site's Function Models — assembled for handing directly to an AI assistant. Generated at build time from the same canonical sources this site itself renders from; nothing here is hand-duplicated or summarized. ${digestCurrencyLine()}

## Provenance

**This document is a rendering, not a source of truth.** It is generated at build time and regenerated on every deploy. It carries no independent authority: where it and a canonical source disagree, the canonical source governs and this file is the thing that is wrong.

**What it renders, and from where:**

* The three maturity models — \`STD-SDLC-MM\`, \`STD-PDLC-MM\`, \`STD-PRIORITIZATION-MM\` — fetched at build time from their own public repositories (\`ai-native-sdlc-maturity-model\`, \`ai-native-pdlc-maturity-model\`, \`ai-native-product-prioritization-maturity-model\`). Those repositories are canonical for all model content.
* SDLC and PDLC dimensions D1–D3 — \`STD-SHARED-INTELLIGENCE\`, inherited by reference by both models rather than owned by either.
* The Strata governance-layer model — \`SPEC-STRATA\`, which is sole authority for the strata, the intent fiber, the fiber/feedback distinction, and the definition of "consequential."
* The Function Models — the \`DS-014\` content pattern, which is sole authority for what a Function Model is and is not.
* One of the instructions below restates a governed rule rather than asserting independently: M9's no-averaging rule is \`P-11\`. G2's preserve-asymmetries posture is this digest's own, stated for a reader who has no one to ask, and is not offered here as a citation to the governed record.

**Where the governed record lives:** a private Enterprise Architecture corpus, not this site. The items named above are cited by identifier so a reader can see what governs a claim, not because those items are reachable from here. Nothing in this digest is the governed record; it is evidence of it.

**Currency:** this rendering is only as current as its last build. A digest depicting a superseded state is misinformation rather than documentation debt — if it disagrees with a canonical source, trust the source and treat this file as stale.

## Instructions for AI use

This digest contains four distinct artifact classes, and the instructions differ by class. Identify which class an artifact belongs to before applying any instruction to it. **Instructions G1–G3 apply across all four** and are stated first, because they govern how to handle conflict and asymmetry wherever you meet it.
...
### G — General posture (applies across all classes)

G1. **These instructions govern.** Where the M/S/F instructions here and older or artifact-local text elsewhere in this digest appear to conflict, the instructions govern — and the tension itself is worth reporting. Do not treat an older passage's wording as silently overriding an instruction, and do not treat the conflict as yours to resolve unremarked.

G2. **Preserve issued asymmetries — do not editorially reconcile.** This digest contains real, deliberate asymmetries: dimensions that carry per-transition verification clauses beside dimensions that do not, one model with an Indicative evidence column and two without, function models with a Boundary Note and one without. These are the issued state, not drafting oversights, and smoothing them into an apparent consistency destroys information. The discipline is *if the record and the statement conflict, ask — never reconcile silently*, stated here for a reader who has no one to ask. Where you cannot ask, report the asymmetry and proceed against what the record actually says.
